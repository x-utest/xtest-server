    ...
    async def get(self):
        ...
        try:
            ...
            if res is True:
                ...
                user_id = "admin"
                res = await User.objects.filter(user_id=user_id).find_all()
                if len(res) == 1:
...
class GetAuthUserOrganization(MyBaseHandler):
    ...
    async def get(self):
        ...
        return get_std_json_response(data=jsontool.dumps(res.to_dict(), ensure_ascii=False))
...
class UpdateUserDetailInfo(MyBaseHandler):
    ...
    async def post(self):
        ...
        # if list_have_none_mem(*[qq, email, phone]):
        #     return ConstData.msg_args_wrong

        user_detail = await UserDetailInfo.objects.get(user=self.get_current_session_user())

        if user_detail is None:  # 如果为空则创建一个
            user_detail = UserDetailInfo()
            user_detail.set_user_tag(http_req=self)
            user_detail.set_default_rc_tag()
            user_detail = await user_detail.save()
...
class GetUserDetailInfo(MyBaseHandler):
    ...
    async def get(self):
        ...
        return get_std_json_response(data=jsontool.dumps(user_detail.to_dict()))
...
class UserLogin(MyBaseHandler):
    ...
    async def post(self):
        ...
        if auth_res:
            token_res = await self.create_token_session(auth_res, WebToken, client_type=ClientTypeDict.browser)
            data = {
                "token": token_res.to_dict()['token']
            }
...
class UserLogout(MyBaseHandler):
    ...
    async def get(self):
        # 记录日志
        # await self.save_logout_history(client_type=ClientTypeDict.browser)
        log_session = self.cache_session
        await log_session.complete_del()
...
class GetAuthUserInfo(MyBaseHandler):
    ...
    @my_async_jsonp
    async def get(self):
        current_auth_user = self.cache_session.user
        """:type:User"""
...
class SetAuthUserInfo(MyBaseHandler):
    ...
    async def post(self):
        ...
        current_auth_user = await User.get_by_obj_id(self.cache_session.user.get_id())
        """:type:User"""
